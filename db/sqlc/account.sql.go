...
SELECT id, owner, balance, currency, created_at FROM accounts LIMIT $1 OFFSET $2
`
...
const transferAccountBalance = `-- name: TransferAccountBalance :one
UPDATE accounts SET balance = balance + $2 WHERE id = $1 RETURNING id, owner, balance, currency, created_at
`

type TransferAccountBalanceParams struct {
	ID      int64 `json:"id"`
	Balance int64 `json:"balance"`
}

func (q *Queries) TransferAccountBalance(ctx context.Context, arg TransferAccountBalanceParams) (Account, error) {
	row := q.db.QueryRowContext(ctx, transferAccountBalance, arg.ID, arg.Balance)
	var i Account
	err := row.Scan(
		&i.ID,
		&i.Owner,
